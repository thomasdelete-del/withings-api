...
<script>
const $=id=>document.getElementById(id), fmtDate=t=>new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'2-digit',year:'numeric'}).format(new Date(t*1000));let shownWeights=[],shownFats=[];
...
async function load(){const key=$('key').value.trim();if(!key)return showError('Bitte den API-Schlüssel einfügen.');$('status').textContent='Lade …';$('error').classList.add('hidden');try{const [weightData,fatData]=await Promise.all([api('/api/v1/weight?limit=10000',key),api('/api/v1/measurements?metric=fat_ratio&limit=10000',key)]);sessionStorage.setItem('withingsApiKey',key);const allWeights=metricItems(weightData,'weight'),allFats=metricItems(fatData,'fat_ratio'),yearAgo=Date.now()/1000-366*86400;shownWeights=allWeights.filter(x=>x.timestamp>=yearAgo);shownFats=allFats.filter(x=>x.timestamp>=yearAgo);if(!shownWeights.length)shownWeights=allWeights;if(!shownFats.length)shownFats=allFats;const latestW=allWeights.at(-1),latestF=allFats.at(-1);$('weight').textContent=latestW?`${latestW.value.toFixed(1)} kg`:'–';$('weightDate').textContent=latestW?fmtDate(latestW.timestamp):'Kein Wert';$('fat').textContent=latestF?`${latestF.value.toFixed(1)} %`:'–';$('fatDate').textContent=latestF?fmtDate(latestF.timestamp):'Kein Wert';const cutoff=Date.now()/1000-30*86400,recent=allWeights.filter(x=>x.timestamp>=cutoff);$('change').textContent=recent.length>1?`${(recent.at(-1).value-recent[0].value)>=0?'+':''}${(recent.at(-1).value-recent[0].value).toFixed(1)} kg`:'–';$('weightRange').textContent=shownWeights.length?`${shownWeights.length} Messungen`:'';$('fatRange').textContent=shownFats.length?`${shownFats.length} Messungen`:'';$('content').classList.remove('hidden');$('login').classList.add('hidden');$('forget').classList.remove('hidden');$('status').textContent='Aktuell';$('updated').textContent=`Geladen: ${new Intl.DateTimeFormat('de-DE',{dateStyle:'medium',timeStyle:'short'}).format(new Date())}`;requestAnimationFrame(renderCharts)}catch(e){$('status').textContent='Fehler';showError(e.message)}}
function renderCharts(){draw($('weightChart'),thin(shownWeights),'#087b5b','kg');draw($('fatChart'),thin(shownFats),'#316b9d','%')}
function showError(msg){$('error').textContent=msg;$('error').classList.remove('hidden')}$('load').onclick=load;$('key').onkeydown=e=>{if(e.key==='Enter')load()};$('forget').onclick=()=>{sessionStorage.removeItem('withingsApiKey');location.reload()};const saved=sessionStorage.getItem('withingsApiKey');if(saved){$('key').value=saved;load()}addEventListener('resize',()=>{if(!$('content').classList.contains('hidden'))renderCharts()});
</script></body></html>'''
